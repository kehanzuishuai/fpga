...
module tb_selfcheck;
  ...
  logic [11:0] leds; logic [6:0] target; logic hit; logic lv, de, da; logic [23:0] latency;
  always #5 clk=~clk;
  ...
  led_guide #(.CLK_HZ(1000)) guide(.clk,.rst_n,.enable(1'b0),.note_played(1'b0),.played_note(7'd0),.leds,.target_note(target),.hit);
  latency_monitor mon(.clk,.rst_n,.sample_tick(tick),.event_strobe(event_stb),.audio(24'sd2048),.dbg_event(de),.dbg_audio(da),.latency_valid(lv),.latency_samples(latency));
  ...
  initial begin
    ...
    morph=0; #1; if(morph_out!=sine) $fatal(1,"morph sine endpoint failed"); morph=1023; #1; if(morph_out!=tri_v) $fatal(1,"morph triangle endpoint failed");
    ...
    @(negedge clk); event_stb=1; @(negedge clk); event_stb=0; do_tick; if(!lv || !da) $fatal(1,"latency/debug pulses failed");
    if(target!=60 || leds!==0 || hit!==0) $fatal(1,"LED/reset check failed");
